...
export default function HomePageKunstwerk(props: {
    ...
    return (
        ...
        <Row style={{ display: "flex", justifyContent: "center" }}>
            <Row>
                <Image className="crop" style={{ cursor: "pointer" }} src={imgUrl} onClick={() => navigate(`/kunstwerk/${props.kunstwerkResource.id}`)} />
            </Row>
            <Row className="pt-4">
                <Col>
                    <div className="fs-5"><strong>{props.kunstwerkResource?.titel}</strong></div>
                    <div className="fs-5">{kuenstler?.name}</div>
                    <div className="fs-5">{angepassterPreis()}</div>
                </Col>
                <Col className="text-end">
                    <Button variant="outline-black rounded-0 my-2 px-4" onClick={() => navigate(`/galerie/${galerie?.id}`)}>
                        {galerie?.name}
                    </Button>
                </Col>
            </Row>
